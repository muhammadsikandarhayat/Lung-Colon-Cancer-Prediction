...
def analyze_dataset():
    # Define paths
    base_path = Path("../../dataset/colon_image_sets")
...
def plot_class_distribution(stats):
    ...
    plt.title('Class Distribution in Colon Cancer Dataset')
    plt.xlabel('Classes')
    plt.ylabel('Number of Images')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'colon_class_distribution.png')
    plt.close()
...
def plot_intensity_distribution(stats):
    ...
    for i, channel in enumerate(['R', 'G', 'B']):
        ...
        plt.legend()
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'colon_intensity_distribution.png')
    plt.close()
...
def plot_quality_metrics(stats):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'colon_quality_metrics.png')
    plt.close()
...
def plot_texture_features(stats):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'colon_texture_features.png')
    plt.close()

if __name__ == "__main__":
    print("Starting Colon Cancer Dataset Analysis...")
